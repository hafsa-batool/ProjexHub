...
const TimeLogs = () => {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/timelogs`, {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/timelogs`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this time log?')) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_URL}/api/timelogs/${id}`, {
          headers: { 'x-auth-token': token }
        });
        fetchLogs();
      } catch (err) {
